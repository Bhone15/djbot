...
const { defprefix, owner } = require('../../config.json');

module.exports = async (client, message) => {
	if (message.author.bot) return;
	if (!message.guild) return;

	const prefix = defprefix;
	...
	if (command) {
		if (command.userperms.length > 0 || command.botperms.length > 0) {
			...
			for(const permission of command.userperms) {
				if(permission === 'BOT_OWNER' && message.member.id !== owner) {
					return;
				}
				else if(!message.member.hasPermission(permission)) {
					return message.channel.send(
						`<:error:853906226097291314> Insufficient Permission! \`${permission}\` required.`,
					);
				}
			}
			...
			for(const permission of command.botperms) {
				if (!message.guild.me.hasPermission(permission)) {
					return message.channel.send(
						`<:error:853906226097291314> Insufficient Permission! I require \`${permission}\`.`,
					);
				}
			}
		}
